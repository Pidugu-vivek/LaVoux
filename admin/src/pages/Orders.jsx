                ...
                {order.review && (
                  <div className='mt-4 text-xs'>
                    ...
                    <div className='flex items-center gap-1 mt-1'>
                      {[...Array(5)].map((_, i) => (
                        <span key={i} className={`text-lg ${i < order.review.deliveryRating ? 'text-yellow-500' : 'text-gray-300'}`}>★</span>
                      ))}
                    </div>
                    {order.review.deliveryComment && <p className='mt-1 text-gray-600'>"{order.review.deliveryComment}"</p>}

                    <h4 className='font-medium mt-3'>Product Feedback</h4>
                    {order.items.map((item) => {
                      ...
                      return productReview ? (
                        <div key={item._id} className='mt-2'>
                          ...
                          {productReview.comment && <p className='text-gray-600'>"{productReview.comment}"</p>}
                        </div>
                      ) : null;
                    })}
                  </div>
